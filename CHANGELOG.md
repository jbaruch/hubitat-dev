...
- The rule and skill now give a real fix for an **unreliable Long Range device at distance** (weak signal / high RTT, intermittently FAILED): re-include it as classic Z-Wave mesh so it routes through nearby mains-powered repeaters, instead of the dead-end "improve placement." Manufacturer-confirmed for Zooz LR switches. Grounded diagnosing a real fleet where 8 identical Zooz LR switches were unreliable with high latency while 65 identical siblings were fine — the differentiator was the marginal direct LR link a star can't repeater around.
