...
3. **RSSI floor, read per hop count.** `lwrRssi` is the last hop **into** the hub (`rules/zwave-zigbee-mesh.md`). It is the node's own radio only on a **direct** link. Skip a **direct** node at/below ~−95 dBm (SiLabs RX floor: −97 dBm 700-series, −110 dBm 800-LR) — floor-signal nodes are hang-prone and rarely flash. On a **routed** node, or one with no readable route, the number is the final repeater's link and this node's own link is **unknown**. Skip that node too; the floor never clears an unmeasured link for a flash. Not worth risking a whole-hub Z-Wave blackout to update one bathroom plug; force either skip only when attended.
