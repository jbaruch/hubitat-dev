...
- **On the zwaveJS backend (2.5.1.140), `per` is not corroborated by the wire — do not rank a node on it alone.** Observed: `per` **decreased** across a session (a cumulative count cannot), and rose +12 over 12 messages the radio log reported as transmit-OK, single-attempt, zero-retry. Confirm a suspected packet-error offender against the transmit log (`hub_radiolog`) before calling it one, and read `packet_errors[]` in `summary.warnings` as a lead, not a verdict. Grounded one hub / one build — the count may still mean what the field name says on other backends or versions.
...
- `listening` — the always-on indicator. `true` identifies a mains-powered classic-mesh node that
  can repeat. `false` identifies a sleepy node.
- Use `listening` instead of guessing repeater capability from device type.
- `beaming` means the node **requires** beam wake-up. It does not mean the node sends beams or repeats.
- The JSON `beaming` field was false even for sampled FLiRS locks on 2.5.1.132.
- Use the Z-Wave Details UI Beaming column when that requirement matters.
...
- **Observed once (n=1, unconfirmed): a rebuild may rebuild routes without refreshing neighbour tables; a hub reboot did.** After adding three repeaters, two full `zwaveRepair2` rebuilds changed 0–1 routes and no pre-existing node's `heard by` list; a hub reboot re-interviewed neighbours (new repeaters went `heard by` 0–2 → 28–33) and routes followed at once. A newly added repeater with high `hears` but near-zero `heard by` in `/hub/zwaveTopology` is an **unpropagated neighbour table**, not a placement fault — nothing has told the fleet it exists, so no route can select it. **Not established:** one hub, one evening, and the reboot coincided with a 2.5.1.134 → 2.5.1.140 update, so reboot and version are confounded. Read `heard by` before calling a fresh repeater badly placed, but verify this before treating "reboot after adding repeaters" as standing guidance.
...
- **On 2.5.1.140, `--summary` emitted no `transmit_report` — the raw transmit lines carried `ACK RSSI` only, no noise floor at either end, so `hub_snr`/`dest_snr` had nothing to parse and this hub-receiver split could not be run.** Either the frame format shifted after the grounded build, or the richer report needs conditions the capture did not hit. Confirm `transmit_report` is present before relying on this split; where it is absent, report the noise-floor question as unmeasurable on this build rather than a hub-receiver verdict. The fields that *are* present on that build — `transmit status`, `routing attempts`, `route speed`, `routing scheme`, `took` — still separate a real drop from a slow S2/FLiRS exchange.
