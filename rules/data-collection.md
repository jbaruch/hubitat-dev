...
- Raise `maxEvents` via `skills/_scripts/hub_device_update.py --set maxEvents=<n>` before relying on the hub for any baseline. 1000 buffers roughly two weeks of a slow-moving signal.
- Never hand-build the `POST /device/update` form — the script owns the field set and the boolean encoding (`rules/device-lifecycle.md`).
