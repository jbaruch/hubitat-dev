...
- **Every boolean is checkbox-semantic — there is no second encoding.** `meshEnabled`, `meshFullSync`, `retryEnabled` and `homeKitEnabled` are each sent as `on` when true and **omitted entirely** when false. Posting the literal string `true` **clears** the field. Mis-encoding is destructive on both pairs: the mesh pair disables hub-mesh sharing, removing the mirrored device on the consuming hub and breaking every app bound to the mirror; `homeKitEnabled` drops the device out of the HomeKit bridge with **no error and no event** (`skills/_reference/homekit-mdns-network.md`). Measured on **2.5.1.135** (2026-07-27): a no-op repost of device 326 sending literal `true` for `retryEnabled` / `homeKitEnabled` flipped **both** to `false` while the checkbox-encoded mesh pair survived. Re-confirmed on **2.5.1.169** (2026-08-30): uniform checkbox encoding round-trips with no drift on a Zigbee moisture sensor and on a hub-mesh mirror. "Serialize all booleans the same way" is the correct replay here — the trap is serializing them as JSON-style literals.
...
**Round-trip a no-op before the first real write against a field set you have not posted before.** Read `fullJson`, rebuild the form from it unchanged, post it, re-read, and diff the two reads. A clean no-op proves your encoding of every field — the booleans above especially — before a real edit rides on it. This is the same read-fresh-then-send discipline `version` already forces, extended to the payload. Two benign diffs are expected on a **freshly linked hub-mesh mirror** and are not drift: the hub normalizes `roomId` `null → 0` and `label` `null → ""`. A mirror is born from `GET /device/createLinked/...` with `name` = source label + `" on <source hub name>"`, `label` = `null`, `roomId` = `null` (2.5.1.169, 2026-08-30).
