...
```
python3 .tessl/plugins/jbaruch/hubitat-dev/skills/_scripts/hub_mesh.py --ip <addr> [--radio zwave|zigbee|both] [--no-probe]
```

Argument contract, output shape, and every flag/rank rule: `skills/_scripts/hub_mesh.py` module docstring.
...
- Zigbee device `active` is not liveness in either direction; long-dead devices can remain
  `active:true`. Use `lastActivity` / `age_seconds`; `active_false_devices[]` is raw metadata only.
...
`fetch_warnings` and `zigbee.activity_unknown[]` are empty, **and** no actuator/reporter asymmetry
appears, report the mesh healthy and finish. If the counters are 0 but actuators are frozen while
reporters are fresh, the mesh is **not** healthy — say so, and proceed. Never report "healthy" on
the counters alone against a symptom of devices not responding, and never over an unmeasured axis.
...
- Z-Wave `listening:true` marks an always-on classic-mesh node that can repeat. `beaming` means the
  device requires beam wake-up, not that it beams for others; the JSON value is unreliable on the
  grounded build, so never use it to select a repeater.
...
`skills/_reference/zwave-lifecycle.md` (TransmitReport).
