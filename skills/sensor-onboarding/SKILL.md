...
Argument contract and output shape: `skills/_scripts/hub_device_update.py` module docstring. On success both calls print one JSON object: `{hub, device_id, mode, form, posted, applied, benign_normalization, unexpected_drift}` — the same keys in every mode. On failure the diagnostic goes to stderr and no JSON is emitted. `applied` holds each field that reached the value asked for, `benign_normalization` the empty-value normalizations the hub performs on its own, `unexpected_drift` everything else. **Exit 0** — the write landed as asked. **Exit 1** — a hub or fetch error, a `version` stamp that did not advance, or a non-empty `unexpected_drift`: the write did not land, so stop and re-read the device rather than repeating the call. **Exit 2** — a config or argument error, nothing was posted. The device edit page is the UI alternative (`skills/_reference/playwright-ui.md`). Proceed to Step 5.
