...
| `hub_device_events.py` | Census a device's retained events per attribute and name the silent channels; `--expect-attribute` asserts one channel is reporting | HTTP |
| `hub_app_subscriptions.py` | Report an installed app's live `eventSubscriptions`; `--expect-device` asserts a device is subscribed after a config Done | HTTP |
